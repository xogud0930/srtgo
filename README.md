...
-- 대상 열차 -----------------------------------------------------------
 > [*] KTX-이음   751  09:20~10:05  일반 가능  특실 매진
   [ ] KTX-이음   753  15:28~16:13  일반 매진  특실 매진  대기 가능
...
| `l` `F7` | 예매 내역 — 결제 대기, 발권 완료, 예약 대기 전부 |
| `t` `F4` | SRT ↔ KTX |
| `r` `F5` | 다시 조회 |
| `p` `F6` | 일시정지 / 재개 |
| `q` `Esc` | 한 단계 뒤로. 대시보드에서 아무것도 없을 때만 종료 |

`l` 로 여는 내역 화면에서는 건마다 `[발권]` `[미결제]` `[대기]` 가 붙고,
`Enter` 로 결제 · 취소 · 환불을 고른다. `g` 는 내역을 텔레그램으로 보낸다.

열차마다 `일반` / `특실` / `대기` 가 **가능**(초록) 인지 **매진**(회색) 인지
따로 표시된다. 화면이 좁으면 뒤쪽(대기 → 특실 → 열차번호) 부터 접고, 조건 요약도
두 칸에서 한 칸으로 바뀐다.
