...
def edit_conditions(rail_type="KTX") -> bool:
    """구간·날짜·시각·승객·좌석·결제를 한 번에 고쳐 keyring 에 저장.

    reserve() 가 읽는 키와 동일해서, 여기서 바꾸면 기존 메뉴에도 그대로 반영된다.
    """
    is_srt = rail_type == "SRT"
    now = datetime.now() + timedelta(minutes=10)
    station_key = get_station(rail_type)[1]
    options = get_options()

    max_days = (30 if is_srt else 31) - (0 if now.hour >= 7 else 1)
    date_choices = [
        ((now + timedelta(days=i)).strftime("%Y/%m/%d %a"),
         (now + timedelta(days=i)).strftime("%Y%m%d"))
        for i in range(max_days + 1)
    ...
    questions = [
        inquirer.List("departure", message="출발역", choices=station_key,
                      default=saved("departure", station_key[0])),
        inquirer.List("arrival", message="도착역", choices=station_key,
                      default=saved("arrival", station_key[-1])),
        inquirer.List("date", message="출발 날짜", choices=date_choices,
                      default=saved("date", date_choices[0][1])),
        inquirer.List("time", message="이 시각 이후",
                      choices=[(f"{h:02d}시", f"{h:02d}0000") for h in range(24)],
                      default=saved("time", "120000")),
        inquirer.List("adult", message="어른 인원", choices=list(range(10)),
                      default=int(saved("adult", "1"))),
    ]
    for key in ("child", "senior", "disability1to3", "disability4to6"):
        if key in options:
            questions.append(
                inquirer.List(key, message=f"{PASSENGER_LABEL[key]} 인원",
                              choices=list(range(10)),
                              default=int(saved(key, "0"))))
    questions += [
        inquirer.List("seat_type", message="좌석 종류", choices=seat_choices,
                      default=saved_seat if saved_seat in [v for _, v in seat_choices]
                      else None),
        inquirer.Confirm("pay", message="예매되면 카드 결제",
                         default=saved("pay", "0") == "1"),
    ]

    answer = inquirer.prompt(questions)
    if answer is None:
        return False
    if answer["departure"] == answer["arrival"]:
        print(colored("출발역과 도착역이 같습니다", "green", "on_red"))
        return False
    if sum(int(answer.get(k, 0)) for k in ("adult", "child", "senior",
                                           "disability1to3", "disability4to6")) < 1:
        print(colored("승객수는 0이 될 수 없습니다", "green", "on_red"))
        return False

    answer["pay"] = "1" if answer["pay"] else "0"
    for key, value in answer.items():
        keyring.set_password(rail_type, key, str(value))
    return True


def settings_menu(rail_type="KTX", debug=False):
    """흩어져 있던 설정을 한 군데로. 로그인이 바뀌면 'login' 을 돌려준다."""
    changed = None
    while True:
        choice = inquirer.list_input(
            message="설정 (Ctrl-C: 돌아가기)",
            choices=[
                (f"{rail_type} 로그인", "login"),
                ("카드", "card"),
                ("역 목록에서 고르기", "station"),
                ("역 직접 입력", "station_edit"),
                ("예매 옵션 / 조회 간격", "options"),
                ("텔레그램 알림", "telegram"),
                ("돌아가기", None),
            ],
        ...
        if choice is None:
            return changed
        if choice == "login":
            set_login(rail_type, debug)
            changed = "login"
        elif choice == "card":
            set_card()
        elif choice == "station":
            set_station(rail_type)
        elif choice == "station_edit":
